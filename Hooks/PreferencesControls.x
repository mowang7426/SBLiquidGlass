...
#import <float.h>
#import <string.h>
#import "../SBLiquidGlassPrefs/LGPrefsLiquidSlider.h"
#import "../SBLiquidGlassPrefs/LGPrefsLiquidSwitch.h"
...
static void *kLGSBLiquidGlassEntryFooterKey = &kLGSBLiquidGlassEntryFooterKey;
...
static BOOL LGIsSBLiquidGlassPreferenceLoaderCell(UITableViewCell *cell) {
    id specifier = nil;
    ...
    for (NSString *key in @[@"lazy-bundle", @"bundle", @"bundlePath"]) {
        ...
        if ([[value description] containsString:@"SBLiquidGlassPrefs"]) return YES;
    }
    ...
    return [title isEqualToString:@"SBLiquidGlass"] &&
           [[detail description] containsString:@"LGPRootListController"];
}

static void LGUpdateSBLiquidGlassEntryFooter(UITableViewCell *cell) {
    UILabel *footer = objc_getAssociatedObject(cell, kLGSBLiquidGlassEntryFooterKey);
    if (!gLGSettingsControlsEnabled) {
        footer.hidden = YES;
        return;
    }
    if (!LGIsSBLiquidGlassPreferenceLoaderCell(cell)) {
        footer.hidden = YES;
        return;
    }
    if (!footer) {
        ...
        objc_setAssociatedObject(cell, kLGSBLiquidGlassEntryFooterKey, footer,
...
static BOOL LGInsideSBLiquidGlassPrefs(UIView *view) {
    for (UIResponder *r = view; r; r = r.nextResponder) {
        if (![r isKindOfClass:UIViewController.class]) continue;
        NSBundle *bundle = [NSBundle bundleForClass:r.class];
        if ([bundle.bundleIdentifier isEqualToString:@"dylv.sbliquidglassprefs"] ||
            [NSStringFromClass(r.class) hasPrefix:@"LG"]) return YES;
    }
    return NO;
}

static BOOL LGControllerContainsSBLiquidGlassPrefs(UIViewController *controller) {
    if (!controller) return NO;
    if ([[NSBundle bundleForClass:controller.class].bundleIdentifier
         isEqualToString:@"dylv.sbliquidglassprefs"] ||
        [NSStringFromClass(controller.class) hasPrefix:@"LG"]) return YES;
    for (UIViewController *child in controller.childViewControllers)
        if (LGControllerContainsSBLiquidGlassPrefs(child)) return YES;
    return controller.presentedViewController &&
           LGControllerContainsSBLiquidGlassPrefs(controller.presentedViewController);
}

static BOOL LGSettingsChromeEnabledForView(UIView *view) {
    if (gLGSettingsControlsEnabled) return YES;
    if (LGInsideSBLiquidGlassPrefs(view)) return YES;
    return LGControllerContainsSBLiquidGlassPrefs(view.window.rootViewController);
}
...
static void LGInstallSettingsSwitch(UISwitch *owner) {
    if (!gLGSwitchControlsEnabled || !owner.window ||
        ...
        LGInsideSBLiquidGlassPrefs(owner)) return;
...
static void LGUpdateSettingsBackButton(UINavigationBar *bar) {
    ...
    BOOL insideSBLiquidGlassPrefs = LGInsideSBLiquidGlassPrefs(bar);
    UIView *content = nil;
    ...
    if (insideSBLiquidGlassPrefs || LGControllerContainsSBLiquidGlassPrefs(navigation) ||
...
static void LGInstallSettingsSegmentGlass(UISegmentedControl *control) {
    if (!gLGSegmentControlsEnabled || !control.window ||
        LGInsideSBLiquidGlassPrefs(control)) return;
...
%group SBLiquidGlassGlobalControls
...
%group SBLiquidGlassPreferencesChrome
...
- (void)layoutSubviews {
    ...
    LGUpdateSBLiquidGlassEntryFooter((UITableViewCell *)self);
}
%end
...
%ctor {
    ...
    %init(SBLiquidGlassGlobalControls);

    if ([bundleIdentifier isEqualToString:@"com.apple.Preferences"])
        %init(SBLiquidGlassPreferencesChrome);
